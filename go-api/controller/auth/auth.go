...
user := orm.User{Username: json.Username, Password:
...
var json LoginBody
if err := c.ShouldBindJSON(&json); err != nil {c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
return
}
...
c.JSON(200, gin.H{"status": "ok", "message": "Login Success!", "token":
tokenString})
} else {
